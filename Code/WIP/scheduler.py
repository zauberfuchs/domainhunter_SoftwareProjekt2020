...
class Scheduler():
    ...
    def delete_rawdata(self):
        try:
            cur = self.db_connection.conn.cursor()
            #rawdata löschen
            datum = str((datetime.now() + timedelta(days=-7)).strftime("%Y/%m/%d"))
            postgres_update_rawdata = 'UPDATE ipv6_whois_query SET raw_data = ''\''+ "" + '\'' + 'WHERE query_time < \'' + datum + '\' AND NOT raw_data = \'\';'

            cur.execute(postgres_update_rawdata)
            postgres_update_rawdata = 'UPDATE ipv4_whois_query SET raw_data = ''\''+ "" + '\'' + 'WHERE query_time < \'' + datum + '\' AND NOT raw_data = \'\';'
            ...
            logger_scheduler.error(f'scheduler.delete_rawdata: database error occurred: {error}')
